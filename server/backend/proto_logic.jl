...
begin
	...
	const ws_messages_log = []; # log for all sent & received (parsed) messages
	const ws_messages_log_OG = []; # log for all received messages (pre-parsing)
...
# HTTP.forceclose(ws_servers_array[end]) 
...
function _mem_cleanup!(force = false) # called by game_runner
    ...
    if force || (time_diff > __cleanup_interval || mem_limit_hit)
		...
		# wipe messages logs
		empty!(ws_messages_log)
		empty!(ws_messages_log_OG)
...
manifest_format = "2.0"
project_hash = "6c93df4d5b8f31f3e2df93071255d8a6ecb87b9f"
